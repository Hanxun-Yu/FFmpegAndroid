 ...
 * description:
 */
public class MultiplePicker extends LinearLayout {
    ...
    public MultiplePicker(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MultiplePicker(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public  void setData(List<PickerData> dataList) {
        removeAllViews();
        if (dataList != null && !dataList.isEmpty()) {
            for (PickerData data : dataList) {
                WheelPicker<String> wheelPicker = new WheelPicker<>(getContext());
                wheelPicker.setHalfVisibleItemCount(2);
                wheelPicker.setDataList(data.data);
                wheelPicker.setIndicatorText(data.unit);
                wheelPicker.setCurrentPosition(data.selectedIndex);
                LinearLayout.LayoutParams params = new LayoutParams(0,-2);
                params.weight = 1;
                wheelPicker.setLayoutParams(params);
                addView(wheelPicker);
                mSubPickers.add(wheelPicker);
            }
        }
        setSubAttr();
    }

    private void setSubAttr() {
    ...
    int textSize, textColor, halfVisibleItemCount, selectedItemTextColor, selectedItemTextSize, itemWidthSpace, itemHeightSpace, curtainColor, curtainBorderColor;
    boolean isTextGradual, isCyclic, isZoomInSelectedItem, isShowCurtain, isShowCurtainBorder;
    ...
    private void initAttrs(Context context, @Nullable AttributeSet attrs) {
        ...
        textSize = a.getDimensionPixelSize(R.styleable.DatePicker_itemTextSize,
                getResources().getDimensionPixelSize(R.dimen.WheelItemTextSize));
        textColor = a.getColor(R.styleable.DatePicker_itemTextColor,
                Color.BLACK);
        isTextGradual = a.getBoolean(R.styleable.DatePicker_textGradual, true);
        isCyclic = a.getBoolean(R.styleable.DatePicker_wheelCyclic, false);
        halfVisibleItemCount = a.getInteger(R.styleable.DatePicker_halfVisibleItemCount, 2);
        selectedItemTextColor = a.getColor(R.styleable.DatePicker_selectedTextColor,
                getResources().getColor(R.color.com_ycuwq_datepicker_selectedTextColor));
        selectedItemTextSize = a.getDimensionPixelSize(R.styleable.DatePicker_selectedTextSize,
                getResources().getDimensionPixelSize(R.dimen.WheelSelectedItemTextSize));
        itemWidthSpace = a.getDimensionPixelSize(R.styleable.DatePicker_itemWidthSpace,
                getResources().getDimensionPixelOffset(R.dimen.WheelItemWidthSpace));
        itemHeightSpace = a.getDimensionPixelSize(R.styleable.DatePicker_itemHeightSpace,
                getResources().getDimensionPixelOffset(R.dimen.WheelItemHeightSpace));
        isZoomInSelectedItem = a.getBoolean(R.styleable.DatePicker_zoomInSelectedItem, true);
        isShowCurtain = a.getBoolean(R.styleable.DatePicker_wheelCurtain, true);
        curtainColor = a.getColor(R.styleable.DatePicker_wheelCurtainColor, Color.WHITE);
        isShowCurtainBorder = a.getBoolean(R.styleable.DatePicker_wheelCurtainBorder, true);
        curtainBorderColor = a.getColor(R.styleable.DatePicker_wheelCurtainBorderColor,
