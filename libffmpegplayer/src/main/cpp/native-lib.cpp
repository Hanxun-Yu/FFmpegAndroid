...
JNIEXPORT jint JNICALL play
    ...
    LOGD("play");
//    const char *file_name = jnicom->strValOf(jnicom->strValOf(path, true), true);
//    LOGD("path:%s", file_name);
//    // sd卡中的视频文件地址,可自行修改或者通过jni传入
//
//    int length = env->GetStringLength(path);
//    for (int i = 0; i < length; i++) {
//        LOGD("char index:%d val:%d", i, file_name[i]);
//    }
    ...
    int error = 0;
    // Open video file
    if ((error = avformat_open_input(&pFormatCtx, file_name, NULL, NULL)) != 0) {
        LOGD("error:%d,Couldn't open file:%s", error, file_name);
